...
- `auto`: default. Select Roslyn/Razor for C#/Razor repos only when supported tooling is available and helper setup is already cached or completes within the first-use setup budget. The default first-use setup budget is 15 seconds for helper build/restore on tiny repos and should be configurable as `--first-use-budget-seconds`; otherwise use `heuristic-fallback` with explicit metadata and diagnostics.
...
| helper build/restore failed or exceeded the first-use setup budget | `helper-setup-failed` |
...
| Auto mode | `codeindex analyze <fixture>` uses Roslyn only when available within cached or first-use setup budget and records truthful C#/Razor modes | 100% | Fail |
